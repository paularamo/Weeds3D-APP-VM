...
#Modified by Paula Ramos 03/2021
...
#Import Information
filename = 'Type Video Name' #'GH010034_NC_Calib.MP4'
...
def ImageCollect(filename, n_boards):
	...
	print('total frames' + str(total_frames))
	    ...
	    image_resize = cv2.resize(image, (960,540))
            ...
            if k == 32:
                collected_images += 1
                #cv2.imwrite('Stream_Image' + str(collected_images) + '.png', image)
 ...
 #'Calibration_Image'
            ...
            #    collected_images += 1
            #    cv2.imwrite('Stream_Image' + str(collected_images) + '.png', image) #'Calibration_Image'
                print(str(collected_images) + ' images collected.')
...
def ImageProcessing(n_boards, board_w, board_h, board_dim):
	...
	image_resize = cv2.resize(image, (1920,1080))
    
        #Converting to grayscale
        #grey_image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
	grey_image = cv2.cvtColor(image_resize, cv2.COLOR_RGB2GRAY)
	cv2.imshow("Gray", grey_image)
        ...
        if found == True:
            ...
            cv2.drawChessboardCorners(image_resize, (board_w, board_h), corners, found)
        
            #Show the image with the chessboard corners overlaid.
            cv2.imshow("Corners", image_resize)
